    ...
    (state, props) => {
        ...
        props.members.map((member) => {
            let memberIssues = filterIssues(props.issues, {members: [member.id]}),
                spent = sumSpentHours(memberIssues, props.issuesSpentTime),
                estimate = sumEstimateHours(memberIssues);
            members[member.id] = Object.assign({}, member, {
                issues: memberIssues.map((issue) => {
                        let spent = sumSpentHours([issue], props.issuesSpentTime),
                ...
                overtime: memberIssues
                    .map((issue) => Math.max(sumSpentHours([issue], props.issuesSpentTime) - sumEstimateHours([issue]), 0))
                    .reduce((a, b) => a + b, 0)
            });
        });
        ...
        return {
            data: data,
            spentHours: sumSpentHours(props.issues, props.issuesSpentTime),
